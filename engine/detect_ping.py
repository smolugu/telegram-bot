...
def detect_ping(
    ...
    # Move the body of the existing backtest
    # 30m loop here.
    #
    # Do NOT change the strategy logic.
    
    last_closed_nq = candle_30m_nq
    last_closed_es = candle_30m_es
    last_closed_candle_timestamp = candle_30m_nq.timestamp
    print("Last closed timestamp: ", last_closed_candle_timestamp)
    # UTC converted timestamps for db queries
    current_30m_start_utc = current_30m_start.astimezone(timezone.utc)
    last_closed_candle_utc = last_closed_candle_timestamp.astimezone(timezone.utc)
    # dt = datetime.fromisoformat(last_closed_candle_timestamp)
    ...
    if dt.hour in [1, 8, 15] and dt.minute == 00:
        ...
        last_3_7h_es = candle_repo.get_last_n(
    ...
    if dt.hour in [22, 2, 6, 10, 14] and dt.minute == 00:
        ...
        last_3_4h_es = candle_repo.get_last_n(
            contract=runtime.es_contract,
            timeframe=240,
            end=current_30m_start_utc,
            n=3,
    ...
    nq_candles_3m_for_auction = candle_repo.get_last_n(
        contract=runtime.nq_contract,
        timeframe=3,
        end=current_30m_start_utc,
        n=40,
    )
    es_candles_3m_for_auction = candle_repo.get_last_n(
        contract=runtime.es_contract,
        timeframe=3,
        end=current_30m_start_utc,
        n=40,
    ...
    refresh_auction(
        auction_engine=runtime.nq_auction_engine,
        candle_30m=last_closed_nq,
    ...
    if dt.hour == 8:
        print("nq auction engine status: ", runtime.nq_auction_engine.status.summary())
    ...
    if window_name != runtime.current_window:
        ...
        if runtime.nq_buy_candidate.alert_sent:
            runtime.nq_buy_candidate.reset()
        if runtime.nq_sell_candidate.alert_sent:
            runtime.nq_sell_candidate.reset()
        if runtime.es_buy_candidate.alert_sent:
    ...
    # this block is not required as we can update session and 7h builder candles
    # from 18:00 and in each 30m loop
    # if (i == 3):
    # if dt.hour == 19 and dt.hour == 30:
    ...
    #     update_weekly_1h_structure_abs(nq_weekly_state = runtime.nq_weekly_state, es_weekly_state= runtime.es_weekly_state, current_30m_start=current_30m_start)
    ...
    if runtime.nq_sell_candidate.active and runtime.nq_sell_candidate.check_breakout_rejection:
        update_sweep_info(runtime.nq_sell_candidate, last_40_candles_3_nq, last_closed_nq)
    if runtime.es_buy_candidate.active and runtime.es_buy_candidate.check_breakout_rejection:
        update_sweep_info(runtime.es_buy_candidate, last_40_candles_3_es, last_closed_es)
    if runtime.es_sell_candidate.active and runtime.es_sell_candidate.check_breakout_rejection:
        update_sweep_info(runtime.es_sell_candidate, last_40_candles_3_es, last_closed_es)
            
    # invalidate failed candidates
    # invalidate of ob is confirmed
    if runtime.nq_buy_candidate.active and runtime.nq_buy_candidate.final_ob_confirmed and runtime.nq_buy_candidate.sweep_candle_extreme > last_closed_nq.low:
        runtime.nq_buy_candidate.reset()
        runtime.es_buy_candidate.reset()
    if runtime.nq_sell_candidate.active and runtime.nq_sell_candidate.final_ob_confirmed and runtime.nq_sell_candidate.sweep_candle_extreme < last_closed_nq.high:
        runtime.nq_sell_candidate.reset()
        runtime.nq_sell_candidate.reset()
    if runtime.es_buy_candidate.active and runtime.es_buy_candidate.final_ob_confirmed and runtime.es_buy_candidate.sweep_candle_extreme > last_closed_es.low:
        runtime.es_buy_candidate.reset()
        runtime.nq_buy_candidate.reset()
    if runtime.es_sell_candidate.active and runtime.es_sell_candidate.final_ob_confirmed and runtime.es_sell_candidate.sweep_candle_extreme < last_closed_es.high:
        runtime.es_sell_candidate.reset()
        runtime.nq_sell_candidate.reset()
    
    # update 7hr candle through seven hour builder
    # he 18:00 7hr candle is not complete with the first 3 30m candles
    runtime.nq_seven_hour_builder.update(last_closed_nq)
    # print("nq 7h after ib ready: ", nq_seven_hour_builder.candles["6PM"].values())   
    runtime.es_seven_hour_builder.update(last_closed_es)
    ...
    session_start = datetime.combine(
        last_closed_candle_timestamp.date(),
        time(18, 0),
        tzinfo=NY_TZ,
    )
    ...
    if dt_current.hour == 1 and dt_current.minute == 00:
        ...
        bullish_nq_ob_level = runtime.nq_buy_candidate.ob_level if runtime.nq_buy_candidate.final_ob_confirmed else None
        bullish_es_ob_level = runtime.es_buy_candidate.ob_level if runtime.es_buy_candidate.final_ob_confirmed else None
        bearish_nq_ob_level = runtime.nq_sell_candidate.ob_level if runtime.nq_sell_candidate.final_ob_confirmed else None
        bearish_es_ob_level = runtime.es_sell_candidate.ob_level if runtime.es_sell_candidate.final_ob_confirmed else None
        add_1am_ob_mitigation_levels(liquidity_levels=runtime.liquidity_nq, bullish_ob_level=bullish_nq_ob_level, bearish_ob_level=bearish_nq_ob_level)
        add_1am_ob_mitigation_levels(liquidity_levels=runtime.liquidity_es, bullish_ob_level= bullish_es_ob_level, bearish_ob_level=bearish_es_ob_level)
        # reset active candidates
        # TODO: dont reset candidates
        # runtime.nq_buy_candidate.reset()
        # runtime.es_buy_candidate.reset()
        # runtime.nq_sell_candidate.reset()
        # runtime.es_sell_candidate.reset()
    ...
    if dt.hour == 2 and dt.minute == 00:
        runtime.nq_london_market_context.set_2am_ib(last_closed_nq)
        runtime.es_london_market_context.set_2am_ib(last_closed_es)
        print("nq 2am IB: ", runtime.nq_london_market_context.ib_2)
        print("es 2am IB: ", runtime.es_london_market_context.ib_2)
    ...
    if dt.hour == 8 and dt.minute == 00:
        ...
        add_8am_ob_mitigation_levels(liquidity_levels=runtime.liquidity_nq, bullish_ob_level=bullish_nq_ob_level, bearish_ob_level=bearish_nq_ob_level)
        add_8am_ob_mitigation_levels(liquidity_levels=runtime.liquidity_es, bullish_ob_level= bullish_es_ob_level, bearish_ob_level=bearish_es_ob_level)
        # reset active candidates
        # TODO: Dont reset candidates (review again)
        # runtime.nq_buy_candidate.reset()
        # runtime.es_buy_candidate.reset()
        # runtime.nq_sell_candidate.reset()
        # runtime.es_sell_candidate.reset()
    ...
    if dt_current.hour == 9 and dt_current.minute == 00:
        ...
        # nq_ny_market_context.structure["bearish_ob_level"] = runtime.nq_sell_candidate.ob_level if runtime.nq_sell_candidate.final_ob_confirmed else None
        # nq_ny_market_context.structure["bullish_ob_level"] = runtime.nq_buy_candidate.ob_level if runtime.nq_buy_candidate.final_ob_confirmed else None
        # es_ny_market_context.structure["bearish_ob_level"] = runtime.es_sell_candidate.ob_level if runtime.es_sell_candidate.final_ob_confirmed else None
        # es_ny_market_context.structure["bullish_ob_level"] = runtime.es_buy_candidate.ob_level if runtime.es_buy_candidate.final_ob_confirmed else None
    ...
    runtime.es_market_context.update_session_range(last_closed_es.high, last_closed_es.low, last_closed_es.open, last_closed_es.close, dt.hour, dt.minute, last_closed_candle_timestamp)
    ...
    if dt.hour > 8 and dt.hour < 15:
        runtime.nq_ny_market_context.update(last_closed_nq, runtime.liquidity_nq)
    ...
    if runtime.nq_market_context.ib_ready:
        runtime.nq_market_context.update_ib_acceptance(last_closed_nq.close)
        runtime.es_market_context.update_ib_acceptance(last_closed_es.close)
        runtime.nq_market_context.compute_expansion_metrics(last_closed_nq.timestamp)
        runtime.es_market_context.compute_expansion_metrics(last_closed_es.timestamp)
        runtime.nq_market_context.update_relative_expansion(runtime.es_market_context.expansion_ratio)
    ...
    sweep_nq_highs_key_level, sweep_nq_lows_key_level = detect_key_liquidity_sweep(instrument = "NQ", key_levels = runtime.liquidity_nq, candles_3m = last_40_candles_3_nq, inside_candles_1m = None, last_closed_candle = last_closed_nq, current_30m_start = current_30m_start)
    sweep_es_highs_key_level, sweep_es_lows_key_level = detect_key_liquidity_sweep(instrument = "ES", key_levels = runtime.liquidity_es, candles_3m = last_40_candles_3_es, inside_candles_1m = None, last_closed_candle = last_closed_es, current_30m_start = current_30m_start)
    ...
    is_compression_nq, compression_flags_nq, compression_range_nq = detect_compression(runtime.nq_seven_hour_builder.candles)
    is_compression_es, compression_flags_es, compression_range_es = detect_compression(runtime.es_seven_hour_builder.candles)
    ...
    if is_post_1AM_IB:
        is_compression_nq, compression_range_nq, compression_sweep_data_nq = runtime.nq_london_market_context.get_compression_data()
        is_compression_es, compression_range_es, compression_sweep_data_es = runtime.es_london_market_context.get_compression_data()
        if dt.hour == 1 and dt.minute == 30:
        ...
        update_compression_range_levels(runtime.liquidity_nq, compression_range_nq, "1AM")
        update_compression_range_levels(runtime.liquidity_es, compression_range_es, "1AM")
    ...
    if is_post_8AM_IB:
        ...
        print("nq market structure ny at each 30m candle: ", runtime.nq_ny_market_context.structure["name"])
        print("es market structure ny at each 30m candle: ", runtime.es_ny_market_context.structure["name"])
    ...
    runtime.nq_market_context.update_compression_info(compression_flags_nq, compression_range_nq)
    runtime.es_market_context.update_compression_info(compression_flags_es, compression_range_es)
    ...
    if is_post_8AM_IB:
        sweep_validation_result = validate_sweeps(
            ...
            nq_ny_market_context = runtime.nq_ny_market_context,
            es_ny_market_context = runtime.es_ny_market_context,
        )
    ...
    if sweep_es_lows or sweep_es_lows_key_level:
        if sweep_es_lows_key_level:
            ...
            # runtime.es_buy_candidate.register_sweep(sweep_es_lows_key_level["timestamp"], sweep_es_lows_key_level["sweep_candle_low"], sweep_es_lows_key_level["sweep_time"], sweep_es_lows_key_level["sweep_and_ob_confirmed"], sweep_es_lows_key_level["sweep_and_ob_entry"], sweep_es_lows_key_level["sweep_and_ob_ce_confirmed"], sweep_es_lows_key_level["sweep_and_ob_ce_entry"], sweep_es_lows_key_level["sweep_and_ob_confirmation_timestamp"], sweep_es_lows_key_level["swept_levels"], "ES", sweep_es_lows_key_level["sweep_type"], sweep_es_lows_key_level["sweep_candle"], sweep_es_lows_key_level["sweep_level"], sweep_es_lows_key_level.get("caution", False))
            if sweep_es_lows_key_level["is_valid"]:
                runtime.es_buy_candidate.register_sweep(sweep_es_lows_key_level)
        elif sweep_es_lows:
            print("Sweep detected ES Lows:", sweep_es_lows)
            # runtime.es_buy_candidate.register_sweep(sweep_es_lows["timestamp"], sweep_es_lows["sweep_candle_low"], sweep_es_lows["sweep_time"], sweep_es_lows["sweep_and_ob_confirmed"], sweep_es_lows["sweep_and_ob_entry"], sweep_es_lows["sweep_and_ob_ce_confirmed"], sweep_es_lows["sweep_and_ob_ce_entry"], sweep_es_lows["sweep_and_ob_confirmation_timestamp"], sweep_es_lows["swept_levels"], "ES", sweep_es_lows["sweep_type"], sweep_es_lows["sweep_candle"], sweep_es_lows["sweep_level"], sweep_es_lows.get("caution", False))
            if sweep_es_lows["is_valid"]:
                runtime.es_buy_candidate.register_sweep(sweep_es_lows)

    #  continue if there are no active candidates
    if not runtime.nq_buy_candidate.active and not runtime.nq_sell_candidate.active and not runtime.es_buy_candidate.active and not runtime.es_sell_candidate.active:
        return

    # print for debug
    if runtime.nq_buy_candidate.active:
        print("Nq Buy candidate active:", runtime.nq_buy_candidate.active,
            "| NQ sweep at:", runtime.nq_buy_candidate.sweep_timestamp, "| NQ ob:", runtime.nq_buy_candidate.ob_data)
    if runtime.nq_sell_candidate.active:
        print("Nq Sell candidate active:", runtime.nq_sell_candidate.active,
            "| NQ sweep at:", runtime.nq_sell_candidate.sweep_timestamp)
    if runtime.es_buy_candidate.active:
        print("Es Buy candidate active:", runtime.es_buy_candidate.active,
            "| ES sweep at:", runtime.es_buy_candidate.sweep_timestamp)
    if runtime.es_sell_candidate.active:
        print("Es Sell candidate active:", runtime.es_sell_candidate.active,
            "| ES sweep at:", runtime.es_sell_candidate.sweep_timestamp)
    ...
    # if runtime.nq_buy_candidate.active and not runtime.nq_buy_candidate.sweep_and_ob_confirmed or runtime.es_buy_candidate.active and not runtime.es_buy_candidate.sweep_and_ob_confirmed:
    last_30_candles_nq = candle_repo.get_last_n(
        contract=runtime.nq_contract,
        timeframe=30,
        end=current_30m_start_utc,
        n=30,
    )
    last_30_candles_es = candle_repo.get_last_n(
        contract=runtime.es_contract,
        timeframe=30,
        end=current_30m_start_utc,
        n=30,
    ...
    if runtime.nq_buy_candidate.active or runtime.es_buy_candidate.active:

        nq_ob = detect_30m_order_block(last_30_candles_nq, runtime.nq_buy_candidate, runtime.es_buy_candidate, last_closed_es)

        if nq_ob:
            runtime.nq_buy_candidate.register_ob(nq_ob)

        es_ob = detect_30m_order_block(last_30_candles_es, runtime.es_buy_candidate, runtime.nq_buy_candidate, last_closed_nq)
        if es_ob:
            runtime.es_buy_candidate.register_ob(es_ob)

    if runtime.nq_sell_candidate.active or runtime.es_sell_candidate.active:
        print("calling detect 30m OB")
        nq_ob = detect_30m_order_block(last_30_candles_nq, runtime.nq_sell_candidate, runtime.es_sell_candidate, last_closed_es)
        print("nq ob: ", nq_ob)
        if nq_ob:
            runtime.nq_sell_candidate.register_ob(nq_ob)

        es_ob = detect_30m_order_block(last_30_candles_es, runtime.es_sell_candidate, runtime.nq_sell_candidate, last_closed_nq)
        print("es ob: ", es_ob)
        if es_ob:
            runtime.es_sell_candidate.register_ob(es_ob)
            print("es sell candidate OB: ", runtime.es_sell_candidate.ob_data)
        
    #  confirmation of sweep on or actual ob detection
    
    if (runtime.nq_buy_candidate.sweep_and_ob_confirmed or runtime.nq_buy_candidate.ob_confirmed):
        runtime.nq_buy_candidate.final_ob_confirmed = True
    if (runtime.nq_sell_candidate.sweep_and_ob_confirmed or runtime.nq_sell_candidate.ob_confirmed):
        runtime.nq_sell_candidate.final_ob_confirmed = True
    if (runtime.es_buy_candidate.sweep_and_ob_confirmed or runtime.es_buy_candidate.ob_confirmed):
        runtime.es_buy_candidate.final_ob_confirmed = True
    if (runtime.es_sell_candidate.sweep_and_ob_confirmed or runtime.es_sell_candidate.ob_confirmed):
        runtime.es_sell_candidate.final_ob_confirmed = True

    # we can continue if no OBs found for active candidates
    should_continue = False
    if (runtime.nq_buy_candidate.active and not runtime.nq_buy_candidate.final_ob_confirmed) and (runtime.es_buy_candidate.active and not runtime.es_buy_candidate.final_ob_confirmed) and (runtime.nq_sell_candidate.active and not runtime.nq_sell_candidate.final_ob_confirmed) and (runtime.es_sell_candidate.active and not runtime.es_sell_candidate.final_ob_confirmed):
    ...
    key_level_bullish_smt_result = detect_bullish_smt_key_levels(runtime.nq_buy_candidate.swept_levels,
        runtime.es_buy_candidate.swept_levels)
    key_level_bearish_smt_result = detect_bearish_smt_key_levels(runtime.nq_sell_candidate.swept_levels,
        runtime.es_sell_candidate.swept_levels)
    ...
    if h1_bullish_smt is not None or h1_bearish_smt is not None:
        ...
        runtime.nq_market_context.update_1h_smt(h1_bullish_smt, h1_bearish_smt)
    # check if smt is holdinh
    runtime.nq_market_context.update_1h_smt_status(last_closed_nq, last_closed_es)
    ...
    # print for debug
    print("Nq Buy candidate OB:", runtime.nq_buy_candidate.ob_confirmed, "| NQ sweep at:", runtime.nq_buy_candidate.sweep_timestamp,
        "| OB data:", runtime.nq_buy_candidate.ob_data, "| Final OB confirmed:", runtime.nq_buy_candidate.final_ob_confirmed)

    print("Nq Sell candidate OB:", runtime.nq_sell_candidate.ob_confirmed, "| NQ sweep at:", runtime.nq_sell_candidate.sweep_timestamp,
        "| OB data:", runtime.nq_sell_candidate.ob_data, "| Final OB confirmed:", runtime.nq_sell_candidate.final_ob_confirmed)

    print("Es Buy candidate OB:", runtime.es_buy_candidate.ob_confirmed, "| ES sweep at:", runtime.es_buy_candidate.sweep_timestamp,
        "| OB data:", runtime.es_buy_candidate.ob_data, "| Final OB confirmed:", runtime.es_buy_candidate.final_ob_confirmed)

    print("Es Sell candidate OB:", runtime.es_sell_candidate.ob_confirmed, "| ES sweep at:", runtime.es_sell_candidate.sweep_timestamp,
        "| OB data:", runtime.es_sell_candidate.ob_data, "| Final OB confirmed:", runtime.es_sell_candidate.final_ob_confirmed)


    fvg = None
    
    if runtime.nq_buy_candidate.final_ob_confirmed and not runtime.nq_buy_candidate.fvg_confirmed:
        ...
        if fvg:
            runtime.nq_buy_candidate.register_fvg(fvg)
            print("Bullish FVG detected:", fvg)
    
    if runtime.nq_sell_candidate.final_ob_confirmed and not runtime.nq_sell_candidate.fvg_confirmed:
        ...
        if fvg:
            runtime.nq_sell_candidate.register_fvg(fvg)
            print("Bearish FVG detected:", fvg)
    
    if runtime.es_buy_candidate.final_ob_confirmed and not runtime.es_buy_candidate.fvg_confirmed:
        ...
        if fvg:
            runtime.es_buy_candidate.register_fvg(fvg)
            print("Bullish FVG detected:", fvg)

    if runtime.es_sell_candidate.final_ob_confirmed and not runtime.es_sell_candidate.fvg_confirmed:
        ...
        if fvg:
            runtime.es_sell_candidate.register_fvg(fvg)
            print("Bearish FVG detected:", fvg)
    # changed from OR to AND, as FVG confirmation is a must, especially during NY open at 9:30
    #  if(runtime.nq_sell_candidate.fvg_confirmed and runtime.nq_sell_candidate.final_ob_confirmed):
    if(runtime.nq_sell_candidate.fvg_confirmed and runtime.nq_sell_candidate.final_ob_confirmed):
        print("NQ Sell candidate ready for alert. FVG confirmed:", runtime.nq_sell_candidate.fvg_confirmed, "| Final OB confirmed:", runtime.nq_sell_candidate.final_ob_confirmed, "| current candle time:", current_30m_start)
        # print("Market Context: ", nq_market_context.values())
    # else:
        # print("NQ Sell candidate NOT ready for alert. FVG confirmed:", runtime.nq_sell_candidate.fvg_confirmed, "| Sweep and OB confirmed:", runtime.nq_sell_candidate.sweep_and_ob_confirmed, "| current candle time:", current_30m_start)
        # print("Market Context: ", nq_market_context.values())
    if(runtime.nq_buy_candidate.fvg_confirmed and runtime.nq_buy_candidate.final_ob_confirmed):
        print("NQ buy candidate ready for alert. FVG confirmed:", runtime.nq_buy_candidate.fvg_confirmed, "| Final OB confirmed:", runtime.nq_buy_candidate.final_ob_confirmed, "| current candle time:", current_30m_start)
        # print("Market Context: ", nq_market_context.values())
    # else:
    #     print("NQ buy candidate NOT ready for alert. FVG confirmed:", runtime.nq_buy_candidate.fvg_confirmed, "| Sweep and OB confirmed:", runtime.nq_buy_candidate.sweep_and_ob_confirmed, "| current candle time:", current_30m_start)
        # print("Market Context: ", nq_market_context.values())
    if(runtime.es_sell_candidate.fvg_confirmed and runtime.es_sell_candidate.final_ob_confirmed):
        print("es Sell candidate ready for alert. FVG confirmed:", runtime.es_sell_candidate.fvg_confirmed, "| Final OB confirmed:", runtime.es_sell_candidate.final_ob_confirmed, "| current candle time:", current_30m_start)
        # print("Market Context: ", es_market_context.values())
    # else:
    #     print("ES Sell candidate NOT ready for alert. FVG confirmed:", runtime.es_sell_candidate.fvg_confirmed, "| Sweep and OB confirmed:", runtime.es_sell_candidate.sweep_and_ob_confirmed, "| current candle time:", current_30m_start)
        # print("Market Context: ", es_market_context.values())
    if(runtime.es_buy_candidate.fvg_confirmed and runtime.es_buy_candidate.final_ob_confirmed):
        print("ES buy candidate ready for alert. FVG confirmed:", runtime.es_buy_candidate.fvg_confirmed, "| Sweep and OB confirmed:", runtime.es_buy_candidate.sweep_and_ob_confirmed, "| Final OB confirmed:", runtime.es_buy_candidate.final_ob_confirmed, "| current candle time:", current_30m_start)
        # print("Market Context: ", es_market_context.values())
    # else:
    #     print("ES buy candidate NOT ready for alert. FVG confirmed:", runtime.es_buy_candidate.fvg_confirmed, "| Sweep and OB confirmed:", runtime.es_buy_candidate.sweep_and_ob_confirmed, "| current candle time:", current_30m_start)
        # print("Market Context: ", es_market_context.values())
    ...
    # group contexts
    es_context = {
        "market_context": runtime.es_market_context,
        "weekly_context": runtime.es_weekly_state,
        "london_context": runtime.es_london_market_context,
        "newyork_context": runtime.es_ny_market_context,
        "liquidity_levels": runtime.liquidity_es,
        "auction_engine": runtime.es_auction_engine,
        
    }
    nq_context = {
        "market_context": runtime.nq_market_context,
        "weekly_context": runtime.nq_weekly_state,
        "london_context": runtime.nq_london_market_context,
        "newyork_context": runtime.nq_ny_market_context,
        "liquidity_levels": runtime.liquidity_nq,
        "auction_engine": runtime.nq_auction_engine,
    ...
    if (runtime.nq_sell_candidate.fvg_confirmed and runtime.nq_sell_candidate.final_ob_confirmed) and not runtime.nq_sell_candidate.alert_sent:
        ...
        if send:
            ...
            # confirmation_dt = to_ny_datetime(runtime.nq_sell_candidate.confirmation_time)
    ...
    if (runtime.nq_buy_candidate.fvg_confirmed and runtime.nq_buy_candidate.final_ob_confirmed) and not runtime.nq_buy_candidate.alert_sent:
        ...
        if send:
            ...
            #     send = False
            # send = True
            print("send == ", send, "trade confirmation time: ", runtime.nq_buy_candidate.confirmation_time, "last_closed_candle: ", last_closed_nq.timestamp)
            # counter trend check later. candidate move might be finished. below condition disallows reversal
    ...
    if (runtime.es_sell_candidate.fvg_confirmed and runtime.es_sell_candidate.final_ob_confirmed) and not runtime.es_sell_candidate.alert_sent:
        # filters
        # session time
    ...
    if (runtime.es_buy_candidate.fvg_confirmed and runtime.es_buy_candidate.final_ob_confirmed) and not runtime.es_buy_candidate.alert_sent:
        ...
        if send:
            ...
            # current_last_closed_dt = to_ny_datetime(last_closed_es["timestamp"])
            # confirmation_dt = to_ny_datetime(runtime.es_buy_candidate.confirmation_time)
            # if confirmation_dt < current_last_closed_dt:
            #     print("current time is ahead of confirmation time, not sending alert")
            #     send = False
            # send = True
    ...
    #     )
    # ================================================================
    # updates that need to be done after processing last closed candle
    # ================================================================
    if dt.hour == 16 and dt.minute == 30:
        print("...")
        # capture current day liquidity in prev_liquidity
        # runtime.prev_liquidity_nq = runtime.liquidity_nq
        # runtime.prev_liquidity_es = runtime.liquidity_es
